"use strict"; // Do not change these configurations after the blockchain is initialized
...
export default {
  // INFO: Usually it is a fee over transaction size (not quantity)
  ...
  pow: {
    getDifficulty: (blocks, index) => {
      ...
      // INFO: The difficulty is the formula that zenny choose to check the proof a work, this number is later converted to base 16 to represent the minimal initial hash expected value.
      ...
      return Math.max(
        Math.floor(
          ...
          Math.pow(
            Math.floor(
              ((index || blocks.length) + 1) / EVERY_X_BLOCKS
            ) + 1,
            POW_CURVE
          )
        )
