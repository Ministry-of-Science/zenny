...
echo "Running zenny inside docker with the following arguments:
  HOST:  $HOST
  PORT:  $PORT
  LOG_LEVEL:  $LOG_LEVEL
  PEERS: $PEERS
...
docker run -t --rm --name zenny_$NAME $HOST_ARG $NAME_ARG $PORT_ARG $PEERS_ARG $LOG_LEVEL_ARG -v /$(pwd):/zenny -p $PORT:$PORT zenny
